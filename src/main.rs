...
fn main() {
    ...
    loop {
        ...
        // ←, ↑, ↓, →
        if keys_0 == "Key1" && keys_1 == "P" { mouse.move_to(mouse_state.coords.0 - 20, mouse_state.coords.1).expect("Unable to move mouse"); }
        if keys_0 == "Key2" && keys_1 == "P" { mouse.move_to(mouse_state.coords.0, mouse_state.coords.1 - 20).expect("Unable to move mouse"); }
        if keys_0 == "Key3" && keys_1 == "P" { mouse.move_to(mouse_state.coords.0, mouse_state.coords.1 + 20).expect("Unable to move mouse"); }
        if keys_0 == "Key4" && keys_1 == "P" { mouse.move_to(mouse_state.coords.0 + 20, mouse_state.coords.1).expect("Unable to move mouse"); }
        // right-click
